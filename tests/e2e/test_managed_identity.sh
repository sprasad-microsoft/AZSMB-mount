...
MOUNT_POINT="$AZSMB_MOUNT_ROOT/mi-$AZSMB_TEST_RUN_ID"
...
mkdir -p "$MOUNT_POINT"
mount -t azsmb "$SOURCE" "$MOUNT_POINT" -o "client_id=$AZSMB_MI_CLIENT_ID"
TEST_DIRECTORY="$MOUNT_POINT/.azsmb-mi-test-$AZSMB_TEST_RUN_ID"
mkdir "$TEST_DIRECTORY"
printf '%s\n' "$AZSMB_TEST_RUN_ID" > "$TEST_DIRECTORY/payload"
[[ "$(cat "$TEST_DIRECTORY/payload")" == "$AZSMB_TEST_RUN_ID" ]]
rm -rf "$TEST_DIRECTORY"

printf 'PASS: managed-identity mount test %s\n' "$AZSMB_TEST_RUN_ID"